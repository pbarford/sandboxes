...
import test.streams.akka.Journal
import test.streams.akka.RabbitMqConsumer.SelectionBet
import test.streams.akka.SelectionAgg.PersistedSelectionBet
import test.streams.akka.fsm.FsmSelectionAgg._
...
class FsmSelectionAgg extends Actor with Stash with FSM[SelectionAggState, SelectionAggData] {
  ...
  when(Running) {
    case Event(s:SelectionBet, r:Rollup) =>
      println("persist")
      journal.bet2(persistHandler(self ! PersistedSelectionBet(s, sender()), persistFailurePrint))
      stay()
